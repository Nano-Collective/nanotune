...
import type { BenchmarkResult, BenchmarkTest, BenchmarkTestResult, MatchMode } from "../types/index.js";
    ...
    switch (mode) {
      ...
      case "startsWith": {
        ...
        if (actualNormalized.startsWith(expectedNormalized)) {
          return { passed: true, matchedAnswer: expected, matchType: "startsWith" };
        }
        break;
      }
      ...
      default: {
        ...
        if (expectedNormalized.startsWith(actualNormalized + " ") ||
...
function generateMarkdownReport(result: BenchmarkResult, systemPrompt: string): string {
  const lines: string[] = [];
  ...
  for (const [category, stats] of Object.entries(result.categories)) {
    ...
    lines.push(`- **${category}:** ${stats.passed}/${stats.total} (${percent}%)`);
  }
  lines.push("");
    ...
    try {
      ...
      for (let i = 0; i < tests.length; i++) {
        ...
        try {
          ...
          latencyMs = Date.now() - startTime;
          response = err instanceof Error ? `Error: ${err.message}` : "Unknown error";
          failures.push({
            id: test.id,
            prompt: test.prompt,
            expected: test.acceptable,
