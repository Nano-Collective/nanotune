...
Run benchmarks against a test dataset. Generates both JSON and Markdown reports with detailed timing metrics.

**Hardware Presets (Quick Start):**
- `--preset <name>` - Use a hardware profile: `low`, `medium`, `high`, or `ultra`

| Preset | Description | Threads | GPU Layers | Context | Batch | Max Tokens |
|--------|-------------|---------|------------|---------|-------|------------|
| `low` | Low-end hardware (older laptops) | 4 | 0 (CPU only) | 2048 | 512 | 50 |
| `medium` | Mid-range hardware (modern laptops) | 8 | 20 | 4096 | 1024 | 100 |
| `high` | High-end (Apple Silicon M1/M2/M3) | auto | max | 8192 | 2048 | 150 |
...
- `--gpu-layers <n>` - GPU layers to offload (default: auto/max)
